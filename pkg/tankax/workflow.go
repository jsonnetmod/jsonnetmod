...
func Apply(loaded Loaded, opts tanka.ApplyOpts) error {
	kube, err := loaded.Connect()
	if err != nil {
		return err
	}
	defer kube.Close()
	...
	// show diff
	diff, err := kube.Diff(loaded.Resources(), kubernetes.DiffOpts{Strategy: opts.DiffStrategy})
	switch {
	case err != nil:
	...
	if !opts.AutoApprove {
		if err := confirmPrompt("Applying to", loaded.Env().Spec.Namespace, kube.Info()); err != nil {
			return err
		}
	}

	return kube.Apply(loaded.Resources(), kubernetes.ApplyOpts{
		Force:    opts.Force,
...
func Delete(l Loaded, opts tanka.DeleteOpts) error {
	kube, err := l.Connect()
	if err != nil {
		return err
	}
...
func Prune(l Loaded, opts tanka.PruneOpts) error {
	kube, err := l.Connect()
	if err != nil {
		return err
	}
